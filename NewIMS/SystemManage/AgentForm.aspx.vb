...
Partial Public Class SystemManag_AgentForm
    Inherits Page
                ...
                If BrRec.Tables(0).Rows.Count = 0 And BranchNo.Text.Trim.IndexOf(" ") < 0 Then
                    ...
                    Else
                        If Len(BranchNo.Text) = 3 And BranchNo.Text.Trim.IndexOf(" ") < 0 Then
                            'If Not CheckBox.Checked Then
                            ExecSql("insert into BranchInfo (BranchName,BranchNo,Address,telephone,FaxNo,eMail,Agent,CompanyOffice) values('" _
                             & BranchName.Text & "','" _
                             & Left(Request("Branch"), 2) & Trim(BranchNo.Text) & "','" _
                             & Address.Text & "','" _
                             & Telephone.Text & "','" _
                             & FaxNo.Text & "','" & eMail.Text & "','" & AgentOrOffice.SelectedItem.Value & "','" & IIf(AgentOrOffice.SelectedItem.Value, 0, 1) & "')")
                            ...
                            ASPxWebControl.RedirectOnCallback("~/SystemManage/AgentForm.aspx?Branch=" + Request("Branch") + "&Agent=" & Left(Request("Branch"), 2) & Trim(BranchNo.Value) & "")
                            'Response.Write("<script>alert('السيارة المطلوبة لديها وثيقة سارية حتى  " + Format(ChckCar.Tables(0).Rows.Item(0).Item("CoverTo"), "yyyy/MM/d") + " برقم " + ChckCar.Tables(0).Rows.Item(0).Item("PolNo") + "');</script>")
                        Else
    ...
    Private Sub AgentUsers_CustomButtonInitialize(sender As Object, e As ASPxGridViewCustomButtonEventArgs) Handles AgentUsers.CustomButtonInitialize
        ...
        Dim TT As Integer = CType(Mid(myList(6), InStr(1, myList(6), "S6") + 3, 1), Short)
        Select Case e.ButtonID
            Case "Activate"
                If Stopped Then
                    e.Enabled = True
