            ...
            Select Case e.Parameter
                ...
                Case "Calc"
                    ...
                    If isValid Then
                        ...
                        Premium.Text = Format(((Rate.Value / 100) * SumIns.Value) * diff, "0.000")
                        If GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")) = 0 Then
                            T = True
                            Premium.Text = Format(((Rate.Value / 100) * SumIns.Value) * diff, "0.000")
                            SetDxtxtValue(FindControlRecursive(PolicyControl, "NETPRM"), Format(((Rate.Value / 100) * SumIns.Value) * diff, "0.000"))
                        ...
                        Else
                            T = False
                            Premium.Text = Format(((Rate.Value / 100) * SumIns.Value), "0.000") 'GetNet(GetEbaValue(PermType, 1), Val(Power.Text), Val(PassNo.Text), Val(Carry.Text))
                            ...
                            SetDxtxtValue(FindControlRecursive(PolicyControl, "NETPRM"), Format(((Rate.Value / 100) * SumIns.Value), "0.000"))
...
PP:
    ...
    Protected Sub ASPxCallbackPanel1_Callback(source As Object, e As CallbackEventArgs)
        Select Case e.Parameter
            Case "SuminsCHANGE"
                e.Result = Convert.ToDecimal(BuildIns.Value) + Convert.ToDecimal(BuildContIns.Value) + Convert.ToDecimal(GoldIns.Value) + Convert.ToDecimal(OwnerIns.Value) + Convert.ToDecimal(Behind.Value) + Convert.ToDecimal(DeathIns.Value)
            Case Else
        End Select
    End Sub
