...
Public Class Fire
    ...
    Protected Sub Callback_Callback(sender As Object, e As CallbackEventArgsBase) Handles Callback.Callback
        ...
        Else
            ...
            GetComboValue(FindControlRecursive(PolicyControl, "Measure"))
            Dim T As Boolean
            ...
            If GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")) <> 0 Or GetComboValue(FindControlRecursive(PolicyControl, "Measure")) <> 3 Then
                diff = diff
            Else
                    ...
                    If isValid Then
                        ...
                        If GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")) = 0 AndAlso Session("Order") <> "" Then
                            ...
                            Using con As New SqlConnection(ConfigurationManager.ConnectionStrings.Item("IMSDBConnectionString").ConnectionString)
                                ...
                                'Else
                                ExecConn("Update " & GetGroupFile(Request("Sys")) & " Set ContRate=ContRate, BuildRate=BuildRate, Premium=Round((((ManBuildIns+BuildIns) * (BuildRate /100)) +((Owner+Behind) * (BuildRate /100)) +((ManBuildCont+BuildCont) * (ContRate /100))) * " & diff & ",3) " _
                                & " where OrderNo='" & GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) & "'" _
                                & " AND EndNo=" & Session("End") & " ", con)
                                'End If
                        ...
                        If GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) = "0" And Request("Order") = "" And MyBase.Session("Order") = "0" Then
                            ...
                            If Page.IsCallback Then
                                ...
                                Callback.JSProperties("cpNewWindowUrl") = "../Fire/FireImport.aspx?OrderNo=" & GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) & "&EndNo=" & GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")) & "&LoadNo=" & GetDxTextValue(FindControlRecursive(PolicyControl, "LoadNo")) & "&Sys=" & Request("Sys") & ""
                            Else
...
PP:                 If GetComboValue(FindControlRecursive(PolicyControl, "PayType")) = 2 And GethiddenField(FindControlRecursive(PolicyControl, "CustNo")) <> 0 Then
    ...
    Protected Function GetNet(Perm As Integer, Power As Integer, PassNo As Integer, Carry As Double) As Double
        'Dim Net As New DataSet
