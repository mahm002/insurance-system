...
Public Class Hinsure
    ...
    Protected Sub Callback_Callback(sender As Object, e As CallbackEventArgsBase) Handles Callback.Callback
        ...
        If IsIssued(GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")), GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")), GetDxTextValue(FindControlRecursive(PolicyControl, "LoadNo")), Request("Sys")) And Not IsSameMonth(GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")), GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")), GetDxTextValue(FindControlRecursive(PolicyControl, "LoadNo")), Request("Sys")) Then
            Exit Sub
        Else
            If GetDxTextValue(FindControlRecursive(PolicyControl, "PolNo")) <> "" Then

                diff = CDbl((DateDiff(DateInterval.Day, CDate(GetDateedit(FindControlRecursive(PolicyControl, "CoverFrom"))), CDate(GetDateedit(FindControlRecursive(PolicyControl, "CoverTo")))) + 1) / GetPolDays(GetDxTextValue(FindControlRecursive(PolicyControl, "PolNo"))))
            Else
                diff = CDbl(DateDiff(DateInterval.Day, CDate(GetDateedit(FindControlRecursive(PolicyControl, "CoverFrom"))), CDate(GetDateedit(FindControlRecursive(PolicyControl, "CoverTo")))) / DateDiff(DateInterval.Day, CDate(GetDateedit(FindControlRecursive(PolicyControl, "CoverFrom"))), DateAdd(DateInterval.Year, 1, CDate(GetDateedit(FindControlRecursive(PolicyControl, "CoverFrom"))))))
            End If
            ...
            diff = If(GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")) <> 0, diff, Math.Ceiling(diff))
                    ...
                    If isValid Then
                        ...
                        If GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")) = 0 AndAlso Session("Order") <> "" Then
                            ...
                            Using con As New SqlConnection(ConfigurationManager.ConnectionStrings.Item("IMSDBConnectionString").ConnectionString)
                                ...
                                Else
                                    ExecConn("Update " & GetGroupFile(Request("Sys")) & " Set Premium=Round(Rate *" & diff & ",3)" _
                                & " where OrderNo='" & GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) & "'" _
                                & " AND EndNo=" & Session("End") & " ", con)
                                End If
                        ...
                        If GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) = "0" And Request("Order") = "" And MyBase.Session("Order") = "0" Then
                            ...
                            If Page.IsCallback Then
                                ...
                                Callback.JSProperties("cpNewWindowUrl") = "../Life/HInsaImport.aspx?OrderNo=" & GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) & "&EndNo=" & GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")) & "&LoadNo=" & GetDxTextValue(FindControlRecursive(PolicyControl, "LoadNo")) & "&Sys=" & Request("Sys") & ""
                            Else
                        ...
                        Else
                            ...
                            ExecConn("Update " & GetGroupFile(Request("Sys")) & " " _
                                   & " Set " & GetGroupFile(Request("Sys")) & ".GroupNo=D.GroupNoT " _
                                   & " From(select serno,ROW_NUMBER() over (partition by S.orderno order by S.CardNo,S.serno) as GroupNoT " _
                                   & " From " & GetGroupFile(Request("Sys")) & " S) D " _
                                   & " Where " & GetGroupFile(Request("Sys")) & ".serno=D.serno " _
                                   & " And " & GetGroupFile(Request("Sys")) & ".orderno='" & GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) & "'", Conn)
                        End If
                    Else
...
PP:                 If GetComboValue(FindControlRecursive(PolicyControl, "PayType")) = 2 And GethiddenField(FindControlRecursive(PolicyControl, "CustNo")) <> 0 Then
    ...
    Protected Function GetNet(ByVal Perm As Integer,
                              ByVal Power As Integer,
                              ByVal PassNo As Integer,
                              ByVal Carry As Double) As Double
        'Dim Net As New DataSet
