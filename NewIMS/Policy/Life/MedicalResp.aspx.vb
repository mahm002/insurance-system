...
Public Class MedicalResp
    ...
    Protected Sub Callback_Callback(sender As Object, e As CallbackEventArgsBase) Handles Callback.Callback
        ...
        Else
            ...
            Dim T As Boolean
            Dim diff As Double = DateDiff(DateInterval.Year, CDate(GetDateedit(FindControlRecursive(PolicyControl, "CoverFrom"))), CDate(GetDateedit(FindControlRecursive(PolicyControl, "CoverTo")))) ' / DateDiff(DateInterval.Day, CDate(GetDateedit(FindControlRecursive(PolicyControl, "CoverFrom"))), DateAdd(DateInterval.Year, 1, CDate(GetDateedit(FindControlRecursive(PolicyControl, "CoverFrom"))))), "0.000"))
            ...
            Select Case e.Parameter
                ...
                Case "Calc"
                    ...
                    If isValid Then
                        ...
                        Premium.Text = Format(GetNet(PlaceOfWork.Tokens.Count) * diff, "0.000")
                        If GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")) = 0 Then
                            T = True
                            Premium.Text = Format(GetNet(PlaceOfWork.Tokens.Count) * diff, "0.000")
                            SetDxtxtValue(FindControlRecursive(PolicyControl, "NETPRM"), Format(GetNet(PlaceOfWork.Tokens.Count) * diff, "0.000"))
                        ...
                        Else
                            T = False
                            Premium.Text = Format(GetNet(PlaceOfWork.Tokens.Count) * diff, "0.000") 'GetNet(GetEbaValue(PermType, 1), Val(Power.Text), Val(PassNo.Text), Val(Carry.Text))
