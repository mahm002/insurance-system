...
Public Class MedicalRespGroup
        ...
        Else
            ...
            Select Case e.Parameter
                    ...
                    If isValid Then
                        ...
                        If GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")) = 0 AndAlso Session("Order") <> "" Then
                            ...
                            Using con As New SqlConnection(ConfigurationManager.ConnectionStrings.Item("IMSDBConnectionString").ConnectionString)
                                ...
                                If RateAll.Checked Then
                                    ExecConn("Update " & GetGroupFile(Request("Sys")) & " Set Premium=Round(" & Rate.Value & " * " & diff & ",3)" _
                                & " where OrderNo='" & GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) & "'" _
                                & " AND EndNo=" & Session("End") & " ", con)
                                    RateAll.Checked = False
                                Else
                                    ExecConn("Update " & GetGroupFile(Request("Sys")) & " Set Premium=Round(dbo.GetSpPremium(Specialize) * " & diff & ",3)" _
                                & " where OrderNo='" & GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) & "'" _
                                & " AND EndNo=" & Session("End") & " ", con)
                                End If
                        ...
                        If GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) = "0" And Request("Order") = "" And MyBase.Session("Order") = "0" Then
                            ...
                            If Page.IsCallback Then
                                ...
                                Callback.JSProperties("cpNewWindowUrl") = "../Life/GroupMedicalImport.aspx?OrderNo=" & GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) & "&EndNo=" & GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")) & "&LoadNo=" & GetDxTextValue(FindControlRecursive(PolicyControl, "LoadNo")) & "&Sys=" & Request("Sys") & ""
                            Else
                ...
                Case "ExRate"
                    ...
                    'SetDxtxtValue(FindControlRecursive(PolicyControl, "ExcRate"), GetExrate(GetComboValue(FindControlRecursive(PolicyControl, "Currency"))))
    ...
    Protected Function GetNet(Sp As Integer) As Double
        ...
        Using con As New SqlConnection(ConfigurationManager.ConnectionStrings.Item("IMSDBConnectionString").ConnectionString)
            ...
            dbadapter.SelectCommand.Parameters.AddWithValue("@Sys", Request("Sys"))
            dbadapter.Fill(Net)

            If Net.Tables(0).Rows.Count <> 0 Then

                Return Net.Tables(0).Rows.Item(0).Item(0)
            Else
    ...
    Protected Sub DataGrid_DataBound(sender As Object, e As EventArgs) Handles DataGrid.DataBound
        Dim gridView = TryCast(sender, ASPxGridView)
