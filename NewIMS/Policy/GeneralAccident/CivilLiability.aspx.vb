...
Public Class CivilLiability
    ...
    Protected Sub ASPxCallbackPanel1_Callback(source As Object, e As CallbackEventArgs)
        Dim callback = CType(source, ASPxCallback)
        Select Case e.Parameter
            Case "SuminsCHANGE"
                e.Result = Convert.ToDecimal(SumInsPerPol.Value) + Convert.ToDecimal(PollutionIns.Value)
            Case Else
        End Select
    End Sub
            ...
            Select Case e.Parameter
                ...
                Case "Calc"
                    ...
                    If isValid Then
                        ...
                        Premium.Text = Format(((Rate.Value / 1000) * SumInsured.Value) * diff, "0.000")
                        If GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")) = 0 Then
                            T = True
                            Premium.Text = Format(((Rate.Value / 1000) * SumInsured.Value) * diff, "0.000")
                            SetDxtxtValue(FindControlRecursive(PolicyControl, "NETPRM"), Format(((Rate.Value / 1000) * SumInsured.Value) * diff, "0.000"))
                        ...
                        Else
                            T = False
                            Premium.Text = Format(((Rate.Value / 1000) * SumInsured.Value) * diff, "0.000") 'GetNet(GetEbaValue(PermType, 1), Val(Power.Text), Val(PassNo.Text), Val(Carry.Text))
                            SetDxtxtValue(FindControlRecursive(PolicyControl, "NETPRM"), Format(((Rate.Value / 1000) * SumInsured.Value) * diff, "0.000"))
