                    ...
                    If isValid Then
                        ...
                        If GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")) = 0 AndAlso Session("Order") <> "" Then
                            ...
                            Using con As New SqlConnection(ConfigurationManager.ConnectionStrings.Item("IMSDBConnectionString").ConnectionString)
                                ...
                                If RateAll.Checked Then
                                    ExecConn("Update " & GetGroupFile(Request("Sys")) & " Set Rate=" & Rate.Value & ", Premium=Round(" & GetEndFile(Request("Sys")) & " *  iif(EmpCnt=1,EmpCnt,1) * (" & Rate.Value & " /1000) * " & diff & ",3)" _
                                & " where OrderNo='" & GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) & "'" _
                                & " AND EndNo=" & Session("End") & " ", con)
                                    RateAll.Checked = False
                                Else
                                    ExecConn("Update " & GetGroupFile(Request("Sys")) & " Set Rate=Rate, Premium=Round(" & GetEndFile(Request("Sys")) & " * iif(EmpCnt=1,EmpCnt,1)  * (Rate /1000) * " & diff & ",3)" _
                                & " where OrderNo='" & GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) & "'" _
                                & " AND EndNo=" & Session("End") & " ", con)
                                End If
                        ...
                        If GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) = "0" And Request("Order") = "" And MyBase.Session("Order") = "0" Then
                            ...
                            If Page.IsCallback Then
                                ...
                                Callback.JSProperties("cpNewWindowUrl") = "../GeneralAccident/PaImport.aspx?OrderNo=" & GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) & "&EndNo=" & GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")) & "&LoadNo=" & GetDxTextValue(FindControlRecursive(PolicyControl, "LoadNo")) & "&Sys=" & Request("Sys") & ""
                            Else
...
PP:                 If GetComboValue(FindControlRecursive(PolicyControl, "PayType")) = 2 And GethiddenField(FindControlRecursive(PolicyControl, "CustNo")) <> 0 Then
    ...
    'Protected Function GetNet(Perm As Integer,
    '                           Power As Integer,
    '                           PassNo As Integer,
    '                           Carry As Double) As Double
    '    'Dim Net As New DataSet
    ...
    Private Sub DataS_Updating(sender As Object, e As SqlDataSourceCommandEventArgs) Handles DataS.Updating
        Dim unused = DataS.UpdateParameters.Add("Period", "1")
