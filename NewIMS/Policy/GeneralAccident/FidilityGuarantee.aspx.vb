...
Public Class FidilityGuarantee
    Inherits Page
            ...
            Select Case e.Parameter
                Case "Endorsment"
                    ...
                    If Page.IsCallback Then
                        ...
                        Callback.JSProperties("cpNewWindowUrl") = "../GeneralAccident/FgImport.aspx?OrderNo=" & GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) & "&EndNo=" & GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")) & "&LoadNo=" & GetDxTextValue(FindControlRecursive(PolicyControl, "LoadNo")) & "&Sys=" & Request("Sys") & ""
                    Else
                        ...
                        If GetDxTextValue(FindControlRecursive(PolicyControl, "EndNo")) = 0 AndAlso Session("Order") <> "" Then
                            ...
                            Using con As New SqlConnection(ConfigurationManager.ConnectionStrings.Item("IMSDBConnectionString").ConnectionString)
                                ...
                                If RateAll.Checked Then
                                    ExecConn("Update " & GetGroupFile(Request("Sys")) & " Set Rate=" & Rate.Value & ", Premium=Round(" & GetEndFile(Request("Sys")) & " * (" & Rate.Value & " /1000) * " & diff & ",3)" _
                                & " where OrderNo='" & GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) & "'" _
                                & " AND EndNo=" & Session("End") & " ", con)
                                    RateAll.Checked = False
                                Else
                                    ExecConn("Update " & GetGroupFile(Request("Sys")) & " Set Rate=Rate, Premium=Round(" & GetEndFile(Request("Sys")) & " * (Rate /1000) * " & diff & ",3)" _
                                & " where OrderNo='" & GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) & "'" _
                                & " AND EndNo=" & Session("End") & " ", con)
                                End If
                            ...
                            Else
                                ...
                                Using con As New SqlConnection(ConfigurationManager.ConnectionStrings.Item("IMSDBConnectionString").ConnectionString)
                                    ...
                                    & " AND EndNo=" & Session("End") & " ", con)
                                        RateAll.Checked = False
                                    Else
                                        ExecConn("Update " & GetGroupFile(Request("Sys")) & " Set Rate=Rate, Premium=Round(SumIns * (Rate /1000) * " & diff & ",3)" _
                                    & " where OrderNo='" & Session("Order") & "'" _
                                    & " AND EndNo=" & Session("End") & " ", con)
                                    End If
                                    Dim dbadapter = New SqlDataAdapter("select Sum(Premium) as Net from " & GetGroupFile(Request("Sys")) & "  where OrderNo='" & GetDxTextValue(FindControlRecursive(PolicyControl, "OrderNo")) & "' And SubIns='" & Request("Sys") & "' " _
